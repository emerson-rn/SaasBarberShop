...
def admin_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if request.user.tipo == 'ADMIN' or request.user.is_staff:
            return view_func(request, *args, **kwargs)
        messages.error(request, 'Acesso permitido apenas para administrador.')
        if request.user.tipo == 'CLIENTE':
            return redirect('portal_cliente')
        if request.user.tipo == 'BARBEIRO':
            return redirect('portal_barbeiro')
...
def lista_equipe(request):
    ...
    context = {
        ...
        'clientes': Usuario.objects.filter(tipo='CLIENTE') if request.user.tipo != 'CLIENTE' else Usuario.objects.filter(pk=request.user.pk),
...
def novo_agendamento(request):
    ...
    if request.method == 'POST':
        try:
            cliente_id = request.user.id if request.user.tipo == 'CLIENTE' else request.POST.get('cliente')
...
def novo_servico(request):
    if request.method == 'POST':
        Servico.objects.create(
            ...
            duracao_minutos=request.POST.get('duracao_minutos') or 30
        )
...
def editar_servico(request, pk):
    ...
    if request.method == 'POST':
        ...
        servico.preco = request.POST.get('preco') or 0
        servico.duracao_minutos = request.POST.get('duracao_minutos') or 30
        servico.save()
        messages.success(request, 'Serviço atualizado com sucesso!')
        return redirect('servicos')
    return render(request, 'servico_form.html', {'servico': servico})
...
def deletar_servico(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('servicos')
    return render(request, 'confirmar_exclusao.html', {'item': servico, 'tipo': 'serviço'})
...
def atualizar_status_agendamento(request, pk):
    ...
    if request.user.tipo == 'CLIENTE':
        return JsonResponse({'success': False, 'error': 'Cliente não pode alterar status.'}, status=403)
    if request.method == 'POST':
        agendamento = get_object_or_404(Agendamento, pk=pk)
...
def confirmar_chegada(request, pk):
    ...
    if request.user.tipo == 'CLIENTE':
        return JsonResponse({'success': False, 'error': 'Cliente não pode confirmar chegada.'}, status=403)
    if request.method == 'POST':
        agendamento = get_object_or_404(Agendamento, pk=pk)
...
def avaliacoes(request):
    """Lista avaliações e mostra estatísticas simples."""
    from django.db.models import Avg, Count

    if request.user.tipo == 'BARBEIRO':
        avaliacoes_qs = Avaliacao.objects.filter(barbeiro=request.user)
    elif request.user.tipo == 'CLIENTE':
        avaliacoes_qs = Avaliacao.objects.filter(cliente=request.user)
    else:
        avaliacoes_qs = Avaliacao.objects.all()

    media_notas = avaliacoes_qs.aggregate(media=Avg('nota'))['media'] or 0
    total_avaliacoes = avaliacoes_qs.count()
    por_barbeiro = Avaliacao.objects.values('barbeiro__username').annotate(
        media=Avg('nota'),
        total=Count('id')
    ).order_by('-media', '-total')[:10]

    context = {
        'avaliacoes': avaliacoes_qs.select_related('cliente', 'barbeiro', 'agendamento'),
...
def nova_avaliacao(request):
    """Cria uma avaliação para um atendimento finalizado."""
    if request.user.tipo == 'CLIENTE':
        agendamentos = Agendamento.objects.filter(
            cliente=request.user,
            status='finalizado'
        ).exclude(avaliacao__isnull=False).select_related('barbeiro', 'servico')
    else:
        agendamentos = Agendamento.objects.filter(
            status='finalizado'
        ).exclude(avaliacao__isnull=False).select_related('cliente', 'barbeiro', 'servico')

    if request.method == 'POST':
        agendamento = get_object_or_404(Agendamento, pk=request.POST.get('agendamento'))

        if request.user.tipo == 'CLIENTE' and agendamento.cliente != request.user:
            messages.error(request, 'Você só pode avaliar os seus próprios agendamentos.')
    ...
    return render(request, 'avaliacao_form.html', {'agendamentos': agendamentos})
